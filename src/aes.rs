...
pub fn decrypt_aes_ecb(ciphertext: &[u8], key: &[u8]) -> Result<Box<[u8]>, String> {
  if ciphertext.len() % 16 != 0 {
    panic!("decrypt_aes_ecb called with invalid text length {}.", ciphertext.len());
  }
  match key.len() {
    16 | 24 | 32 => (),
    _ => panic!("decrypt_aes_ecb called with invalid key length {}.", key.len())
  }
  ...
  let size_written = decrypter.update(ciphertext, &mut out)
      .map_err(|e| e.to_string())?;

  let mut decrypted = Vec::new();
  decrypted.extend_from_slice(&out[0..size_written]);

  let final_size = decrypter.finalize(&mut out)
      .map_err(|e| e.to_string())?;
...
pub fn decrypt_aes_cbc(ciphertext: &[u8], key: &[u8], iv: [u8; AES_BLOCK_SIZE])
